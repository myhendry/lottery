...
describe.only("Lottery unit test", function () {
  ...
  it("Test if users can enter a lottery that is not open", async function () {
    await expect(
      this.lottery.enter((overrides = { value: 250000000000000 }))
  ...
  it("Test if users can enter a lottery that is open", async function () {
    ...
    await this.lottery
      ...
      .enter((overrides = { value: ethers.utils.parseUnits("5", 16) }));
    await this.lottery
  ...
  it("Test if the winner is chosen correctly", async function () {
    ...
    this.mockVRFCoordinator.callBackWithRandomness(
      requestId,
      STATIC_RNG,
